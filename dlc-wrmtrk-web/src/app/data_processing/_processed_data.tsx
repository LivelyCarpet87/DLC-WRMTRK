...
const VideoTile = memo(function VideoTile({md5}:{md5:string}){
    ...
    } else if (proc_state.includes("warning")){
        proc_indicator = (
            <Tooltip label={proc_state} openDelay={300}>
    ...
    } else if (proc_state.includes("failed")){
        proc_indicator = (
            <Tooltip label={proc_state} openDelay={300}>
                    ...
                    <Table.Td>
                        {
                            ...
                            :
                            <Tooltip label="Infrequent inconsistencies detected." openDelay={700}>
    ...
    return (
        <div className="border-2 border-slate-300 rounded-md p-3 flex flex-col gap-2 w-104">
            <div className="flex flex-row gap-2 items-center">
                <Button unstyled className="w-fit text-inherit" onClick={()=>window.open(videoSWR.data!.raw_download)}>
                    <p className="text-ellipsis w-90 text-left">{videoSWR.data.filename}</p>
                </Button>
                {proc_indicator}
            </div>
            {dataDisplay}
...
function PlateTile({uuid}:{uuid:UUID}){
    ...
    return (
        <div className="bg-slate-100 rounded-md p-3 flex flex-col gap-4 w-112">
            <div className="flex flex-row gap-4 w-104 items-center justify-between">
                <p className="text-ellipsis w-50">Batch ID: {plateSWR.data!.plateID}</p>
                <div className="flex flex-row gap-2 items-center">
                    {condTags}
                </div>
            </div>
