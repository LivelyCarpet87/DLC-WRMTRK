...
def track_data_processing(vidMD5):
    ...
    for indv in [f"ind{i}" for i in range(1,numInd+1)]:
        ...
        if np.isnan(np.array(longest_tracklet[2])).sum() > len(longest_tracklet[2])/4 or len(longest_tracklet[2]) < len(range(min_frame+step_size,max_frame+1,step_size))/3:
            ...
            confidence = False
        elif memCur.execute('SELECT AVG(confidence) from labels WHERE indiv = ? AND frame_num >= ? AND frame_num < ?', [indv,longest_tracklet[0],longest_tracklet[1]]).fetchone()[0] < 0.75:
    ...
    for frame_ind in range(min_frame+step_size,max_frame+1,step_size):
        ...
        for indv in [x[0] for x in filter(lambda x: frame_ind in range(x[1][0],x[1][1]),label_ind_bounds)]:
            ...
            for i in range(2,len(parts_list)-1):
                pointQ1 = memCur.execute('SELECT x_pos, y_pos FROM labels WHERE frame_num = ? AND indiv = ? AND bodypart = ?  AND x_pos != NULL and y_pos != NULL', [frame_ind, indv, parts_list[i-1]]).fetchone()
                pointQ2 = memCur.execute('SELECT x_pos, y_pos FROM labels WHERE frame_num = ? AND indiv = ? AND bodypart = ?  AND x_pos != NULL and y_pos != NULL', [frame_ind, indv, parts_list[i]]).fetchone()
